...
@click.option("--only-self-constructing", is_flag=True, default=False, help="Only run self-constructing shields.")
def main(results_file, log_file, model_path, episode_length, num_environments,
         num_parallel_environments, agent, nu, number_of_evaluations, uniform_random_agent, model_checking_eval, goal_rew, fail_rew, deterministic_agent, shield_file_name, shield_memory, only_self_constructing):
    ...
    if only_self_constructing:
        considerred_shields = ["self-constructing-safe", "self-constructing-unsafe", "constructed-self-constructing-safe", "constructed-self-constructing-unsafe"]
    else:
