...
class Shield:
    ...
    def correct(self, last_action: any, current_state: any, distribution):
...
class IdentityShield(Shield):
    ...
    def correct(self, last_action, current_state, distribution, reset):
        self.shield_calls += 1
...
class PessimisticShield(Shield):
    ...
    def correct(self, last_action, current_state, distribution, reset):
        ...
        if reset:
            ...
            assert self.last_state is not None, "Last state is None on non-reset."
            assert self.last_distr is not None, "Last distribution is None on non-reset."
            last_action = last_action[0]
            last_action_label = self.actions[last_action]

            prev_state_choice_labels = []

            for choice in range(self.model_info.model.transition_matrix.get_row_group_start(self.last_state), self.model_info.model.transition_matrix.get_row_group_end(self.last_state)):
            ...
            row_index = self.model_info.model.transition_matrix.get_row_group_start(self.last_state) + last_choice_index
            row = self.model_info.model.transition_matrix.get_row(row_index)
            ...
            self.path_prob = self.path_prob * last_transition_prob * self.last_distr[last_choice_index]


        qmax = self._qmax(current_state, distribution)
        this_step_safety = self.path_prob * (self.model_info.vmax[current_state] - qmax)
        self.incurred_safety += this_step_safety
        ...
        if self.incurred_safety >= self.bmax:
            ...
            this_step_safety = self.path_prob * (self.model_info.vmax[current_state] - qmax)
            self.incurred_safety += this_step_safety
            self.last_distr = new_distribution
...
class OptimisticShield(Shield):
    ...
    def correct(self, last_action, current_state, distribution, reset):
        ...
        this_step_risk = self.path_prob * (qmin - self.model_info.vmin[current_state])
        self.incurred_risk += this_step_risk
...
class SelfConstructingShield(Shield):
    ...
    def correct(self, last_action, current_state, distribution, reset):
        ...
        if reset:
            ...
            prev_state_choice_labels = []
            for choice in range(self.model_info.model.transition_matrix.get_row_group_start(self.current_node.state_index), self.model_info.model.transition_matrix.get_row_group_end(self.current_node.state_index)):
                prev_state_choice_labels.append(self.model_info.model.choice_labeling.get_labels_of_choice(choice).pop())

            last_choice_index = prev_state_choice_labels.index(last_action_label)
            last_action = last_choice_index

            if (current_state, last_action) not in self.current_node.successors.keys():
                self.current_node.successors[(current_state, last_action)] = Node({}, self.current_node, last_action, [], current_state, self.model_info.vmin[current_state])
                self.current_node = self.current_node.successors[(current_state, last_action)]
            else:
                self.current_node = self.current_node.successors[(current_state, last_action)]
        ...
        if not self.point_in_convex_hull(self.current_node.distributions + self.vmin_actions_distributions[current_state], distribution):
            self.current_node.distributions.append(distribution)
...
class SelfConstructingShieldDistributions(SelfConstructingShield):
    ...
    def correct(self, last_action, current_state, distribution, reset):
        ...
        if reset:
            ...
            if (current_state, self.last_distribution_index) not in self.current_node.successors.keys():
                all_dist = self.vmin_actions_distributions[self.current_node.state_index] + self.current_node.distributions
                assert self.last_distribution_index is None and len(all_dist) == 0 or self.last_distribution_index < len(all_dist)
                self.current_node.successors[(current_state, self.last_distribution_index)] = Node({}, self.current_node, self.last_distribution_index, [], current_state, self.model_info.vmin[current_state])
                self.current_node = self.current_node.successors[(current_state, self.last_distribution_index)]
            else:
                self.current_node = self.current_node.successors[(current_state, self.last_distribution_index)]
        ...
        if output_distribution in self.vmin_actions_distributions[current_state]:
            self.last_distribution_index = self.vmin_actions_distributions[current_state].index(output_distribution)
        elif output_distribution in self.current_node.distributions:
            self.last_distribution_index = len(self.vmin_actions_distributions[current_state]) + self.current_node.distributions.index(output_distribution)
        else:
            self.current_node.distributions.append(output_distribution)
            self.last_distribution_index = len(self.vmin_actions_distributions[current_state]) + len(self.current_node.distributions) - 1
