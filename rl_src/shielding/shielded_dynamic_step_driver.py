...
@gin.configurable
class ShieldedDynamicStepDriver(driver.Driver):
  ...
  def __init__(
    ...
    Raises:
      ValueError:
        ...
        tf_policy.TFPolicy.
    """
    super(ShieldedDynamicStepDriver, self).__init__(
  ...
  def _loop_body_fn(self):
    ...
    def loop_body(counter, time_step, policy_state):
      ...
      action_step = self.policy.action(time_step, policy_state) # What the agent originally performs
      ...
      logits = self.shield_processor.compute_new_logits(
          ...
          prev_actions=self.prev_actions.numpy().tolist(),
  ...
  def _run(self, time_step=None, policy_state=None, maximum_iterations=None):
    ...
    self.prev_actions = tf.zeros(batch_dims, self.env.action_spec().dtype)
