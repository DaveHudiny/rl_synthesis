    ...
    for current_iter in range(0, 256, 4):
        ...
        if ';' not in last_line:
            ...
            with open(plot_data_file, "a") as f:
                if not file_exists:
                    f.write("model;agent;shield;shield_memory;nu;iter;risk;reward;shield_calls;blocked_actions;earliest_shielded_step\n")
                actual_agent = "uniform-random" if uniform_random_agent else agent
                f.write(f"{os.path.basename(os.path.normpath(model_path))};{actual_agent};{shield_type};{shield_memory};{nu};{current_iter};{risk};{reward};{shield_calls};{blocked_actions};{earliest_shielded_step}\n")
