...
def model_check_given_policy_and_shield(state_to_actions, shield, episode_length=50, goal_value=100.0, antigoal_value=-100.0, expected_shield_calls=False):
    ...
    if type(shield) in [compact_rl.rl.shielding.shields.OptimisticShield, compact_rl.rl.shielding.shields.PessimisticShield]:
        assert False, "Model checking not possible for optimistic or pessimistic shields."
    if type(shield) in [compact_rl.rl.shielding.shields.SelfConstructingShieldOnline, compact_rl.rl.shielding.shields.SelfConstructingShieldOffline]:
        assert False, "Model checking not possible for construction algorithms."
    if type(shield) in [compact_rl.rl.shielding.shields.ShieldWithBudget]:
        assert False, "Model checking not possible for shields with a risk budget function."
    ...
    if type(shield) in [compact_rl.rl.shielding.shields.IdentityShield, compact_rl.rl.shielding.shields.StandardShield, compact_rl.rl.shielding.shields.DeltaShield]:
    ...
    if type(shield) in [compact_rl.rl.shielding.shields.SelfConstructingShield]:
        ...
        if shield.memory > 0:
            ...
            standard_shield = compact_rl.rl.shielding.shields.StandardShield(shield.model_info, shield.actions)
